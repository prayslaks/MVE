...
void AMVE_StageLevel_ChatManager::SendMessage(const FString& MessageContent)
{
	// 빈 메시지 무시
	...
	// 클라이언트 측에서 고유 ID 생성
	FGuid ClientMessageID = FGuid::NewGuid();

	// 서버로 전송 요청
	ServerSendMessage(MessageContent, ClientMessageID);

	PRINTLOG(TEXT("Client: Sending message '%s' with ID %s"), *MessageContent, *ClientMessageID.ToString());
}
...
void AMVE_StageLevel_ChatManager::BroadcastSystemMessage(const FString& SystemMessage)
{
	if (!HasAuthority())
	{
		PRINTLOG(TEXT("BroadcastSystemMessage can only be called on server!"));
		return;
	}
	...
	PRINTLOG(TEXT("Server: Broadcasting system message '%s'"), *SystemMessage);
}

void AMVE_StageLevel_ChatManager::ServerSendMessage_Implementation(const FString& MessageContent,
	const FGuid& ClientMessageID)
{
	if (!HasAuthority())
	{
		return;
	...
	if (!PC)
	{
		// Owner가 없으면 첫 번째 PlayerController 사용
		PC = GetWorld()->GetFirstPlayerController();
	}
	
	if (!PC || !PC->PlayerState)
	{
		PRINTLOG(TEXT("Server: Cannot find PlayerState for message sender"));
		return;
	}
	...
	{
		PRINTLOG(TEXT("Server: Rate limit exceeded for player %s"), *PlayerState->GetPlayerName());
	...
	{
		PRINTLOG(TEXT("Server: Message validation failed: %s"), *RejectReason);
	...
	PRINTLOG(TEXT("Server: Broadcasting message '%s' from %s"), *MessageContent, *SenderName);
...
bool AMVE_StageLevel_ChatManager::ServerSendMessage_Validate(const FString& MessageContent,
	const FGuid& ClientMessageID)
{
	// 기본 검증 (악의적인 요청 차단)
	return !MessageContent.IsEmpty() && MessageContent.Len() <= MaxMessageLength * 2;
}

void AMVE_StageLevel_ChatManager::MulticastBroadcastMessage_Implementation(const FChatMessage& Message)
{
	PRINTLOG(TEXT("Received message: [%s] %s"), *Message.SenderName, *Message.MessageContent);
...
{
	PRINTLOG(TEXT("Message %s removed: %s"), 
...
{
	PRINTLOG(TEXT("ChatHistory replicated: %d messages"), ChatHistory.Num());

	// 신규 접속자를 위한 히스토리 로드 처리
	// UI는 델리게이트를 통해 업데이트됨
...
bool AMVE_StageLevel_ChatManager::CheckRateLimit(APlayerState* Player)
{
	...
	if (Bucket.Tokens >= 1.0f)
	{
		...
		PRINTLOG(TEXT("Rate limit check passed for %s (Tokens: %.2f)"), 
			*Player->GetPlayerName(), Bucket.Tokens);
		
		return true;
	}

	PRINTLOG(TEXT("Rate limit exceeded for %s (Tokens: %.2f)"), 
		*Player->GetPlayerName(), Bucket.Tokens);
...
void AMVE_StageLevel_ChatManager::FilterMessageAsync(const FChatMessage& Message)
{
	...
	Request->OnProcessRequestComplete().BindLambda(
		[this, MessageID = Message.MessageID](FHttpRequestPtr HttpRequest, FHttpResponsePtr HttpResponse, bool bSucceeded)
		{
			...
			{
				PRINTLOG(TEXT("AI Filter request failed for message %s"), 
					*MessageID.ToString());
			...
			if (FJsonSerializer::Deserialize(Reader, ResponseJson) && ResponseJson.IsValid())
			{
				...
				PRINTLOG(TEXT("AI Filter result for %s: %s (Reason: %s)"), 
					*MessageID.ToString(), 
			...
			{
				PRINTLOG(TEXT("Failed to parse AI Filter response"));
				OnAIFilterCompleted(MessageID, true, TEXT(""));
			}
		}
	);
	...
	PRINTLOG(TEXT("AI Filter request sent for message %s"), 
...
void AMVE_StageLevel_ChatManager::OnAIFilterCompleted(const FGuid& MessageID, bool bPassed, const FString& Reason)
{
	...
	if (!FoundMessage)
	{
		PRINTLOG(TEXT("Message %s not found in pending list"), 
			*MessageID.ToString());
		return;
	}
	...
	if (bPassed)
	{
		...
		PRINTLOG(TEXT("Message %s passed AI filter, added to history"), 
			*MessageID.ToString());
	}
	else
	{
	...
	[&](APlayerState* PS)
		...
		PRINTLOG(TEXT("Message %s blocked by AI filter: %s"), 
			*MessageID.ToString(), *Reason);
	}
...
void AMVE_StageLevel_ChatManager::AddToChatHistory(const FChatMessage& Message)
{
	...
	if (ChatHistory.Num() > MaxChatHistory)
	{
		...
		PRINTLOG(TEXT("Removed %d old messages from history"), RemoveCount);
	}

	PRINTLOG(TEXT("Message added to history (Total: %d)"), ChatHistory.Num());
}
