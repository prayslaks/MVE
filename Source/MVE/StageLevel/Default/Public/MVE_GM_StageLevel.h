...
public:
	...
	/**
	 * 카메라 플래시 효과를 처리합니다.
	 * @param FlashManController 플래시를 터뜨린 플레이어의 컨트롤러
	 * @param IgnoreActors
	 ...
	 */
	void HandleFlashEffect(const AController* FlashManController, const TArray<AActor*>& IgnoreActors, const FVector& FlashLocation, const FVector& FlashDirection, float EffectiveDistance, float FlashAngleDotThreshold) const;

protected:
	UPROPERTY(EditDefaultsOnly, Category = "Flash Effect")
	float PlayerLookAtFlashDotThreshold = 0.6f;
