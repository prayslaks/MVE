...
void UMVE_StageLevel_WidgetController_Chat::Initialize(bool bAutoFindManager)
{
	if (!GetWorld())
	{
		PRINTLOG_CHAT(TEXT("Initialize failed: Invalid World"));
		return;
	}
...
void UMVE_StageLevel_WidgetController_Chat::Shutdown()
{
	...
	PRINTLOG_CHAT(TEXT("ChatWidgetController shutdown"));
}
...
void UMVE_StageLevel_WidgetController_Chat::SetChatManager(AMVE_StageLevel_ChatManager* InChatManager)
{
	...
	if (ChatManager)
	{
		...
		// 신규 접속자를 위한 초기 히스토리 로드
		TArray<FChatMessage> History = ChatManager.Get()->GetChatHistory();
		for (const FChatMessage& Message : History)
		{
			OnChatMessageAdded.Broadcast(Message);
		}

		PRINTLOG_CHAT(TEXT("Connected to ChatManager, loaded %d messages from history"), History.Num());
	}
}
...
bool UMVE_StageLevel_WidgetController_Chat::SendMessage(const FString& MessageContent)
{
	if (!ChatManager)
	{
		PRINTLOG_CHAT(TEXT("SendMessage failed: ChatManager not connected"));
	...
	{
		PRINTLOG_CHAT(TEXT("Local validation failed: %s"), *RejectReason);
	...
	{
		PRINTLOG_CHAT(TEXT("Pending queue full"));
	...
	PendingMsg.Timestamp = FDateTime::Now();
	PendingMsg.MessageID = FGuid::NewGuid();

	// 로컬 PlayerController에서 UserName 가져오기
	if (APlayerController* PC = GetWorld()->GetFirstPlayerController())
	{
		if (AMVE_PC_StageLevel* StagePC = Cast<AMVE_PC_StageLevel>(PC))
		{
			PendingMsg.SenderName = StagePC->GetUserName();
		}
	}

	// UserName이 없으면 기본값 설정
	if (PendingMsg.SenderName.IsEmpty())
	{
		PendingMsg.SenderName = TEXT("나");
	}

	AddPendingMessage(PendingMsg);

	// 서버로 전송 (동일한 MessageID 사용)
	ChatManager.Get()->SendMessage(MessageContent, PendingMsg.MessageID);

	PRINTLOG_CHAT(TEXT("Message sent: '%s' (ID: %s)"),
...
{
	PRINTLOG_CHAT(TEXT("Message received: [%s] %s"),
	...
	{
		// 내가 보낸 메시지가 확정됨
		PRINTLOG_CHAT(TEXT("Pending message confirmed: %s"),
...
{
	PRINTLOG_CHAT(TEXT("Message removed: %s (Reason: %s)"),
...
AMVE_StageLevel_ChatManager* UMVE_StageLevel_WidgetController_Chat::FindChatManager()
{
	...
	for (TActorIterator<AMVE_StageLevel_ChatManager> It(GetWorld()); It; ++It)
	{
		...
		if (Manager)
		{
			PRINTLOG_CHAT(TEXT("Found ChatManager: %s"), *Manager->GetName());
			return Manager;
		}
	}
...
void UMVE_StageLevel_WidgetController_Chat::AddPendingMessage(const FChatMessage& Message)
{
	...
	PRINTLOG_CHAT(TEXT("Added to pending queue (Total: %d)"), PendingMessages.Num());
}
...
TOptional<FChatMessage> UMVE_StageLevel_WidgetController_Chat::RemovePendingMessage(const FGuid& MessageID)
{
	...
	if (FoundIndex != INDEX_NONE)
	{
		...
		PRINTLOG_CHAT(TEXT("Removed from pending queue (Remaining: %d)"), PendingMessages.Num());
