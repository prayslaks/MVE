...
void UMVE_WC_Chat::NativeConstruct()
{
	...
	PRINTLOG(TEXT("ChatWidget constructed"));
}
...
void UMVE_WC_Chat::NativeDestruct()
{
	...
	PRINTLOG(TEXT("ChatWidget destructed"));
}
...
void UMVE_WC_Chat::SetController(UMVE_StageLevel_WidgetController_Chat* InController)
{
	...
	if (Controller)
	{
		...
		PRINTLOG(TEXT("Controller set and history loaded"));
	}
}
...
void UMVE_WC_Chat::OnMessageAdded(const FChatMessage& Message)
{
	...
	PRINTLOG(TEXT("Message added to ListView: [%s] %s"), *Message.SenderName, *Message.MessageContent);
}
...
void UMVE_WC_Chat::OnSendFailed(const FString& Reason, EChatRejectReason RejectType)
{
	...
	PRINTLOG(TEXT("Send failed: %s"), *Reason);
}
...
void UMVE_WC_Chat::OnMessageRemoved(const FGuid& MessageID)
{
	...
	PRINTLOG(TEXT("Message removed from ListView: %s"), *MessageID.ToString());
...
bool UMVE_WC_Chat::SendMessage(const FString& MessageContent)
{
	if (!Controller)
	{
		PRINTLOG(TEXT("SendMessage failed: Controller not set"));
...
void UMVE_WC_Chat::LoadChatHistory()
{
	if (!Controller)
	{
		PRINTLOG(TEXT("LoadChatHistory: Controller not set"));
		return;
	}
	...
	PRINTLOG(TEXT("Loaded %d messages from history"), History.Num());
...
void UMVE_WC_Chat::ClearChat()
{
	...
	PRINTLOG(TEXT("Chat cleared"));
}
void UMVE_WC_Chat::AddMessageToListView(const FChatMessage& Message)
{
	if (!MessageListView)
	{
		PRINTLOG(TEXT("AddMessageToListView: MessageListView not bound"));
		return;
	}
	...
	if (Entry)
	{
		...
		PRINTLOG(TEXT("Message entry added (Total: %d)"), MessageEntries.Num());
	}
}
...
void UMVE_WC_Chat::RemoveMessageFromListView(const FGuid& MessageID)
{
	...
	if (FoundEntry && *FoundEntry)
	{
		...
		PRINTLOG(TEXT("Message entry removed (Remaining: %d)"), MessageEntries.Num());
	}
}
...
void UMVE_WC_Chat::ShowError(const FString& ErrorMessage)
{
	...
	{
		// 없으면 로그로만 출력
		PRINTLOG(TEXT("Error: %s"), *ErrorMessage);
	}
}
...
void UMVE_WC_Chat::SetupInputBox()
{
	if (!MessageInputBox)
	{
		PRINTLOG(TEXT("MessageInputBox not bound"));
		return;
	}
	...
	PRINTLOG(TEXT("InputBox setup complete"));
}
...
void UMVE_WC_Chat::SetupSendButton()
{
	if (!SendButton)
	{
		PRINTLOG(TEXT("SendButton not bound"));
		return;
	}
	...
	PRINTLOG(TEXT("SendButton setup complete"));
}
