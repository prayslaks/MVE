...
void ATestGenAIActor::BeginPlay()
{
    ...
    // SR->SendGenerationRequest(TEXT("가벼운 깃털 같은 천사"),TEXT("/Https/UserEmail"),TEXT("api/OpenLoadFileSecetion"));
    // 자동 테스트 실행
    if (bAutoTest)
    {
...
void ATestGenAIActor::TestSendRequest()
{
    ...
    SR->SendGenerationRequest(
        TEXT("귀엽고 깜찍하게 웃고있는 행복하고 검은 곰돌이"),
        TEXT("test@example.com"),
        TEXT("C:/Users/user/Desktop/bear.jpeg") 
    );
